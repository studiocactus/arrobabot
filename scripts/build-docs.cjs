...
function render(md,chapter){
 const lines=md.replace(/\r/g,'').split('\n');const html=[];let i=0;const headingIds=new Map();
...
const content=chapters.map(c=>'<article id="'+c.id+'" data-chapter><div class="chapter-label">'+escape(c.title)+'</div>'+render(fs.readFileSync(path.join(docs,c.file),'utf8'),c)+'<a class="back" href="#top">Voltar ao início ↑</a></article>').join('\n');
const nav=chapters.map((c,i)=>'<a data-nav="'+c.id+'" href="#'+c.id+'"><span>'+String(i+1).padStart(2,'0')+'</span>'+escape(c.title)+'</a>').join('');
