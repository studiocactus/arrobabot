...
type Row={scope:string;name:string;value:unknown;userId:string};
export function VariableTarget({value,onChange}:{value:string;onChange:(v:string)=>void}){
 const dot=value.indexOf('.');const scope=dot<0?'local':value.slice(0,dot);const name=dot<0?value:value.slice(dot+1);
 return <div className="variable-target"><Field label="Guardar o valor"><select value={scope} onChange={e=>onChange(e.target.value+'.'+name)}>{Object.entries(scopeNames).filter(([key])=>key!=='data').map(([key,label])=><option value={key} key={key}>{label}</option>)}</select></Field><Field label="Nome do valor" hint="Use letras sem acentos, números e sublinhado. Exemplo: contador."><input value={name} onChange={e=>onChange(scope+'.'+e.target.value)} placeholder="contador" maxLength={64}/></Field></div>
}
export function VariableManager({profileId,onClose}:{profileId:string;onClose:()=>void}){
 const [rows,setRows]=useState<Row[]>([]);const [userId,setUserId]=useState('');const [scope,setScope]=useState('global');const [name,setName]=useState('');const [value,setValue]=useState('');const [error,setError]=useState('');const [busy,setBusy]=useState(false);
 const [valueType,setValueType]=useState('text');
 async function refresh(){if(!desktop)return;setBusy(true);try{setError('');setRows(await api<Row[]>('variables.list',{profileId,userId}))}catch(e){setError(errorText(e))}finally{setBusy(false)}}
 useEffect(()=>{setRows([])},[userId]);useEffect(()=>{void refresh()},[profileId]);
 async function change(operation:string,target=scope+'.'+name){setBusy(true);setError('');try{let parsed:unknown=value;if(operation!=='delete'){if(valueType==='number'){if(!value.trim()||!Number.isFinite(Number(value)))throw Error('Informe um número válido, usando ponto nas casas decimais.');parsed=Number(value)}else if(valueType==='boolean')parsed=value==='true';else if(valueType==='json')parsed=JSON.parse(value)}await api('variables.change',{profileId,userId,target,operation,value:parsed});await refresh()}catch(e){setError(errorText(e))}finally{setBusy(false)}}
 function editRow(r:Row){setScope(r.scope);setName(r.name);setValueType(typeof r.value==='string'?'text':typeof r.value==='number'?'number':typeof r.value==='boolean'?'boolean':'json');setValue(typeof r.value==='string'?r.value:JSON.stringify(r.value))}
 return <Modal wide title="Variáveis do perfil" onClose={onClose}><div className="form-pad"><p>Organize textos, metas e contadores compartilhados entre comandos deste perfil. Globais e por usuário sobrevivem ao fechamento; variáveis de sessão são apagadas ao reiniciar.</p><p className="help">Não use este espaço para senhas ou tokens. Os valores ficam no banco local e podem aparecer nas respostas do bot.</p>
 <Field label="ID do usuário (para variáveis por pessoa)"><input disabled={busy} value={userId} onChange={e=>setUserId(e.target.value)} placeholder="ID da plataforma; não o nome exibido"/></Field><button type="button" disabled={!desktop||busy} onClick={()=>void refresh()}>Carregar variáveis</button>
 {!desktop&&<p className="help">O gerenciamento de variáveis está disponível no aplicativo desktop.</p>}
 <div className="form-grid"><Field label="Escopo"><select value={scope} onChange={e=>setScope(e.target.value)}><option value="global">Perfil · persistente</option><option value="user">Pessoa · persistente</option><option value="session">Perfil · sessão</option><option value="sessionUser">Pessoa · sessão</option></select></Field><Field label="Nome da variável"><input value={name} onChange={e=>setName(e.target.value)} placeholder="contador" maxLength={64}/></Field></div>
 <Field label="Tipo de valor"><select value={valueType} onChange={e=>{setValueType(e.target.value);if(e.target.value==='boolean')setValue('false')}}><option value="text">Texto</option><option value="number">Número / contador</option><option value="boolean">Sim ou não</option><option value="json">Lista ou objeto (JSON avançado)</option></select></Field><Field label="Valor">{valueType==='boolean'?<select value={value} onChange={e=>setValue(e.target.value)}><option value="true">Sim</option><option value="false">Não</option></select>:<textarea rows={3} value={value} onChange={e=>setValue(e.target.value)}/>}</Field><button type="button" className="primary" disabled={!desktop||busy||!name||(['user','sessionUser'].includes(scope)&&!userId)} onClick={()=>void change('set')}>Salvar variável</button>
 {error&&<p role="alert" className="inline-error">{error}</p>}
 <div className="table-scroll"><table><thead><tr><th>Variável</th><th>Valor</th><th>Ações</th></tr></thead><tbody>{rows.map(r=><tr key={r.scope+'.'+r.name}><td><strong>{r.name}</strong><small>{scopeNames[r.scope]}</small></td><td className="variable-cell">{typeof r.value==='string'?r.value:typeof r.value==='boolean'?(r.value?'Sim':'Não'):JSON.stringify(r.value)}</td><td><button type="button" disabled={busy} onClick={()=>editRow(r)}>Editar</button><button type="button" disabled={busy} className="danger" onClick={()=>{if(window.confirm('Apagar '+r.name+'?'))void change('delete',r.scope+'.'+r.name)}}>Apagar</button></td></tr>)}</tbody></table></div>{!rows.length&&<p className="help">Nenhuma variável carregada. Informe um ID e clique em carregar para incluir dados por pessoa.</p>}
 </div></Modal>
}
