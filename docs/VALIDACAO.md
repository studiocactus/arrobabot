...
## Prévia fiel ao gatilho e nome sorteado de espectador — 0.1.10

Executados após a correção da prévia de variáveis para o gatilho de timer e a criação de `{{randomViewer}}`: `npm run check` (TypeScript), onze testes Vitest, sete testes dos scripts de atualização (`npm run test:updates`), dez cenários Playwright no Edge, 54 testes Rust (`cargo test --locked --lib -j 1`), `npm run build` e o gerador do manual com vinte e um capítulos, com o Playwright reexecutado logo em seguida, e `npm run update:check` aprovando versões e registros.

A causa do erro relatado foi confirmada no código: `Variables.tsx` enviava sempre `kind:'chat'`, `user:'Ana'` e `userId:'test-user'`, e `lib.rs` usava esse evento sem consultar o gatilho, enquanto `timers.rs` monta o disparo real com `user:"BotLive"` e `user_id` vazio. A prévia de um timer, portanto, mostrava um nome que a produção nunca entrega. `model::preview_event` passou a entregar o evento do gatilho antes de construir o contexto: o timer usa o mesmo construtor de `timers.rs`, incluindo `timerId` e `timerRevision`, a voz entrega a voz local do streamer, comando e mensagem contém reescrevem a mensagem de teste para começar pelo texto que dispara, e os demais gatilhos recebem o tipo de evento escolhido. O botão **Simular timer**, que refez o evento à mão e ficava sem `timerRevision`, passou a usar a mesma construção. O diálogo de comando, o de timer e o editor visual passaram a mandar o fluxo aberto para o editor de mensagem, de modo que a prévia do editor simples calcula ações na ordem, condições e variáveis locais como o editor visual já fazia; em gatilho de timer, os campos de teste deixam de aparecer e a explicação do evento real ocupa o lugar deles.

`{{randomViewer}}` lê `community.names`, cadastro que `modules::chat` preenche a cada mensagem recebida e que é apagado em cascata junto com o perfil. O sorteio acontece em `Context::new`, portanto uma vez por execução. Com o cadastro vazio o marcador é ausente e a renderização devolve o erro documentado de variável ausente; com `|default:` devolve a alternativa.

Três testes novos cobrem a correção: `model::tests::preview_event_follows_the_trigger`, para o evento de timer (BotLive, sem ID, mensagem vazia e `simulated` preservado), de comando (`!minecraft` no lugar do `!oi` de teste), de contém, de voz e de seguidor; `integration_tests::timer_preview_delivers_the_real_event_and_sorts_a_chatter`, que verifica cadastro vazio → `{{randomViewer}}` ausente e `{{randomViewer|default:alguém}}` → "alguém", depois de uma mensagem recebida → "Ana", e prévia com dados de teste de chat → `BotLive||timer`; e a asserção de rótulo de catálogo em `variableCatalog.test.ts`. No Playwright, `tests/ui/timers.spec.ts` passou a abrir a prévia de um timer e conferir que a explicação do evento real está visível, que os campos de teste não existem e que **Conferir variáveis** segue desabilitado fora do aplicativo desktop, com o resultado em `artifacts/timer-preview-note.png`.

Limites registrados: o cadastro só tem quem escreveu no chat; o sorteio não pondera por frequência nem por tempo e não garante não repetir a mesma pessoa em disparos consecutivos; `{{userId}}` vazio num timer continua não interrompendo a ação, porque a variável existe no contexto e só chega como string vazia — o que mudou foi a prévia deixar de esconder isso. Nenhuma mensagem foi publicada em chat durante a validação e o cálculo da prévia não envia nada. O cálculo foi conferido pelo teste de integração, que chama o mesmo endpoint usado pelo aplicativo desktop; uma conferência manual dentro do aplicativo desktop não foi executada nesta máquina.
