...
test('manual organiza tarefas, exemplos e referência sem quebrar links antigos',async({page})=>{
 ...
 await expect(page.locator('article')).toHaveCount(21);await expect(page.locator('.nav-group')).toHaveCount(4);
 ...
 await page.getByLabel('Encontrar no manual').fill('contagem regressiva');await expect(page.locator('article:visible')).toHaveCount(1);await page.getByRole('button',{name:'Limpar busca'}).click();await expect(page.locator('article:visible')).toHaveCount(21);
 ...
 await page.emulateMedia({media:'print'});await expect(page.locator('article:visible')).toHaveCount(21);expect(errors).toEqual([]);
});
